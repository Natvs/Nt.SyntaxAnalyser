...
[<CompiledName("CreateRule")>]
let public create_rule (terminals: TokensList) (nonTerminals: TokensList) (symbolIndex:int) (derivation:GrammarToken list) : Rule =
    let rule = new Rule(terminals, nonTerminals)
    rule.SetToken(symbolIndex, -1)
    derivation
...
[<CompiledName("AddNonTerminalToDerivation")>]
...
[<CompiledName("ExpandRuleDerivation")>]
...
[<CompiledName("AddRuleToGrammar")>]
let public add_rule_to_grammar (g: Grammar) (rule: Rule) = g.Rules.Add(rule)

/// Extends the name of a token at a given index
[<CompiledName("ExtendToken")>]
